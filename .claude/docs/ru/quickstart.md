...
Замените `my-project` на имя вашего проекта. Агент выполнит все три шага — вам нужно только подтверждать его действия.
...
Коллеги, склонировавшие репозиторий, выполняют `git submodule update --init` для получения фреймворка, затем `python .tausik-lib/bootstrap/bootstrap.py --smart` для настройки локальной `.tausik/`.
...
Агент:
1. Проверит код — запустит тесты, линтеры, quality gates
